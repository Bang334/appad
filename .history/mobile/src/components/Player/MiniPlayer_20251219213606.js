...
const MiniPlayer = ({ bottomOffset }) => {
  ...
  // If bottomOffset is undefined (for tab screens), calculate from tab bar height
  // If bottomOffset is provided (for stack screens), use it but ensure it's above safe area
  const calculatedBottom = bottomOffset !== undefined 
    ? Math.max(bottomOffset, insets.bottom || 0) // Ensure above safe area for stack screens
