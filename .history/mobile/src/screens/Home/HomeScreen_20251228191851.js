...
const HomeScreen = ({ navigation, route }) => {
  ...
  const onRefresh = async () => {
    ...
    setRefreshing(true);
    hasReachedEnd.current = false; // Reset flag khi refresh
  ...
  // Handler để detect scroll và tự động reload khi đến cuối
  const handleScroll = (event) => {
    ...
    // Detect scroll direction (xuống = positive, lên = negative)
    const scrollDelta = scrollY - lastScrollY.current;
    lastScrollY.current = scrollY;
    
    // Chỉ reload khi scroll xuống và gần đến cuối trang
    // Khi còn cách cuối 300px hoặc đã đến cuối
    const distanceFromEnd = contentHeight - (scrollY + scrollViewHeight);
    const isScrollingDown = scrollDelta > 0;
    const isNearEnd = distanceFromEnd < 300;
    
    // Reload khi scroll xuống và gần đến cuối, chưa reload, và không đang refresh
    if (isScrollingDown && isNearEnd && !hasReachedEnd.current && !refreshing && !isDragging) {
      hasReachedEnd.current = true;
      console.log('📜 [SCROLL] Reached end, triggering refresh...');
      onRefresh().then(() => {
        // Reset flag sau 3 giây để có thể reload lại nếu cần
        setTimeout(() => {
