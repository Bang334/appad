...
export const artistService = {
  ...
  createSong: async (artistId, songData, files = null) => {
    ...
    if (files) {
      try {
        if (files.audio) {
          ...
          // Always use duration from upload response if available
          // This ensures duration is extracted and saved
          if (audioRes.data.duration) {
            finalSongData.duration = Math.round(audioRes.data.duration);
            console.log(`✅ Duration extracted from upload: ${finalSongData.duration}s`);
          } else {
            console.warn('⚠️ No duration returned from upload, backend will try to extract');
          }
        }
    ...
    // Ensure duration is set - if not provided and file_url exists, backend will extract it
    if (!finalSongData.duration && finalSongData.file_url) {
      console.log('ℹ️ No duration provided, backend will extract from file_url');
    }
  ...
  updateSong: async (artistId, songId, songData, files = null) => {
    ...
    if (files) {
      try {
        if (files.audio) {
          ...
          if (audioRes.data.duration) {
            finalSongData.duration = Math.round(audioRes.data.duration);
            console.log(`✅ [updateSong] Duration extracted from upload: ${finalSongData.duration}s`);
          } else {
            console.warn('⚠️ [updateSong] No duration returned from upload, backend will try to extract');
          }
        }
    ...
    // Ensure duration is set - if not provided and file_url exists, backend will extract it
    if (!finalSongData.duration && finalSongData.file_url) {
      console.log('ℹ️ [updateSong] No duration provided, backend will extract from file_url');
    }
