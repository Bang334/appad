...
const AdminArtistReviewsScreen = ({ route, navigation }) => {
  ...
  const loadSongs = async () => {
    try {
      const response = await adminService.getArtistSongs(artistId);
      if (response.success) {
  ...
  const loadReviews = async () => {
    try {
      ...
      const response = await adminService.getArtistReviews(artistId, params);
      if (response.success) {
    ...
    <View style={styles.container}>
      <View style={[styles.header, { paddingTop: Math.max(insets.top + 20, 60) }]}>
        ...
        <View style={{ flex: 1, alignItems: 'center' }}>
            <Text style={styles.headerTitle} numberOfLines={1}>{artistName || 'Đánh giá từ khán giả'}</Text>
            <Text style={styles.headerSubtitle}>Quản lý đánh giá</Text>
        </View>
        <View style={{ width: 40 }} /> 
      </View>
...
export default AdminArtistReviewsScreen;
